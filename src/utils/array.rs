...
pub(crate) fn new_checked<const N: usize, T: Copy + Debug + Default>(
...
fn fill_mut<const N: usize, T: Debug>(mut hydrate: [T; N], it: impl Iterator<Item = T>) -> [T; N] {
    hydrate.iter_mut().zip(it).for_each(|(i, v)| *i = v);
    hydrate
}

pub(crate) fn new<const N: usize, T>(mut it: impl Iterator<Item = T>) -> Result<[T; N]> {
